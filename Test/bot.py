...
async def addCertificate(ctx, member: discord.Member, year):
    ...
    # damned = ref.child(f'{member.name}').get()
    j = 0
    # for i in damned.values():
...
@client.command()
async def How_you_doin(ctx,member:discord.Member):
    await ctx.send(f'{ctx.message.author.name}, at your service da')
...
client.run('ODQ0ODYzMDE0MTE2MjYxOTAx.YKYl_w.-c-XiT7O2ogJeJs4J9o42HMcPqM')